...
Despite the name, this is a **zero-Python** pure-Rust rebuild of a deterministic small-language-model training pipeline. Python-language source files are *input data only* (curated as a training corpus). No Python interpreter, package, code generator, or Python-launched subprocess may appear anywhere in build, data prep, training, verification, or receipts. Permitted native code is limited to three named boundaries by `SCOPE-002` (`docs/decision-ledger-v4.md`): the pinned `tree-sitter-python 0.25.0` generated C parser behind the Rust data-policy boundary, feature-gated accelerator backends, and the Parquet Zstandard decoder (`zstd-sys`) reached only through the Parquet reader in `src/stack.rs`, decode-only, because the Stack v2 metadata `SOURCE-001` names is Zstandard-compressed and no pure-Rust backend exists.
...
- **Corpus acquisition is contract-required, not forbidden.** `AGENTS.md` (Security & Configuration) mandates the contract's HTTPS, redirect, path-containment, and hash-chain rules, and `docs/rebuild-contract.md:109` marks HTTPS/checksums/credentials `REIMPLEMENT` — "mandatory in production". Fetching is HTTPS-only with bounded redirects, every byte hashed against a pinned digest before publication, plain HTTP allowed only in an explicit local-fixture mode (`:110`), bounded streaming rather than download-all (`:113`), and credentials read only from named environment variables. Acquisition landed late (E3), so most of the pipeline still starts from local bytes; that is implementation order, not a boundary.
