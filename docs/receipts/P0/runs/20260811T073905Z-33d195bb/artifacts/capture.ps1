...
if (Test-Path -LiteralPath $runsRoot) {
    Get-ChildItem -Recurse -File -LiteralPath $runsRoot |
        ...
        ForEach-Object {
            ...
            $preexistingRunFiles.Add($repoRelative, $hash)
        }
}
...
$targetRoot = Join-Path ([IO.Path]::GetTempPath()) (
    'python-slm-p0-' + [guid]::NewGuid().ToString('N')
)
...
function Invoke-JsonValidationCapture {
    ...
    try {
        Get-ChildItem -LiteralPath . -Filter '*.example.json' -File |
            Sort-Object Name |
            ForEach-Object {
                [void](Get-Content -Raw -Encoding UTF8 -LiteralPath $_.FullName |
                    ConvertFrom-Json)
                $names.Add($_.Name)
            }
    }
...
try {
    ...
    $observedPreexisting = [Collections.Generic.Dictionary[string,string]]::new(
        [StringComparer]::Ordinal
    )
    Get-ChildItem -Recurse -File -LiteralPath $runsRoot |
        ...
        ForEach-Object {
            ...
            $repoRelative = $repoRelative.Replace('\', '/')
            $hash = (Get-FileHash -Algorithm SHA256 -LiteralPath $_.FullName).Hash
            $observedPreexisting.Add($repoRelative, $hash.ToLowerInvariant())
        }
    if ($observedPreexisting.Count -ne $preexistingRunFiles.Count) {
        throw 'pre-existing receipt evidence file set changed during capture'
    }
    foreach ($entry in $preexistingRunFiles.GetEnumerator()) {
        if (-not $observedPreexisting.ContainsKey($entry.Key) -or
            $observedPreexisting[$entry.Key] -ne $entry.Value) {
            throw "pre-existing receipt evidence changed: $($entry.Key)"
        }
    }
    $preexistingEvidenceUnchanged = $true
    ...
    if (-not $targetFull.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) -or
        -not ([IO.Path]::GetFileName($targetFull)).StartsWith('python-slm-p0-')) {
    ...
    $summary = [ordered]@{
        schema = 'python-slm-p0-capture-v3'
        run_id = $runId
    ...
    try {
        ...
        try {
            ...
            $seal = $null
            $failureMessage += '; finalization retry: ' + (
